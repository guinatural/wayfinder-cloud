################################################################################
# Wayfinder Cloud  Outputs: dev environment
################################################################################

output "audit_bucket_id" {
  description = "Nome do bucket S3 de trilha de auditoria"
  value       = module.storage.audit_bucket_id
}

output "kms_key_arn" {
  description = "ARN da KMS CMK principal"
  value       = module.storage.kms_key_arn
  sensitive   = false # ARN não é sensível
}

output "critical_topic_arn" {
  description = "ARN do SNS Topic para alertas críticos"
  value       = module.notifications.critical_topic_arn
}

output "wayfinder_event_bus_arn" {
  description = "ARN do EventBridge Event Bus wayfinder-events"
  value       = module.observability.wayfinder_event_bus_arn
}

output "cloudwatch_dashboard_url" {
  description = "URL do CloudWatch Dashboard Wayfinder Command Center"
...
output "athena_workgroup" {
  description = "Nome do Athena workgroup para consultas de auditoria"
...
output "securityhub_arn" {
  description = "ARN do Security Hub habilitado"
  value       = module.security.securityhub_arn
}

output "inspector_enabled" {
  description = "Inspector v2 habilitado (true/false)"
